...
public class CompilePineToDotNetTests
{
    ...
    public void Test_compile_specialized_for_kernel_head()
    {
        ...
        compiledFormattedExpression.Syntax.ToFullString().Should().Be(
    ...
    {
            new ExprMappedToParentEnv.PathInParentEnv([]).Should().Be(
                new ExprMappedToParentEnv.PathInParentEnv([]));

            new ExprMappedToParentEnv.PathInParentEnv([1, 3]).Should().Be(
    ...
    public void Test_CompileEnvItemsPathsForExprFunction()
    {
        ...
        foreach (var testCase in testCases)
        {
            ...
            {
                paths[i].Should().BeEquivalentTo(testCase.expectedPaths[i], options => options.WithStrictOrdering(), $"Path {i}");
            }
        }
    }
